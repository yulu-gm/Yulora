...
编辑器主界面不再以单张卡片或整页设置切换作为主要呈现方式，而是采用更接近桌面编辑器的壳层结构：

- 最左侧保留窄侧轨，承担品牌、入口占位与设置入口
- 中间是随窗口宽度自适应的工作区画布，作为文档编辑主区域
- 设置页改为从左侧滑入的抽屉，不再通过整页切换打断当前文档上下文
- 底部或低优先级区域提供状态条，持续展示保存状态、自动保存状态和字数等信息

该壳层的设计目标是保持写作工具的克制感，同时为后续大纲、最近文件和搜索等能力预留自然版位，但本轮不实现这些功能。视觉细节中的字号、颜色和字体仍由主题系统与偏好设置控制，布局层只负责信息层级与容器关系。
