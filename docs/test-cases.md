...
步骤：
1. 打开设置页。
2. 在“应用 UI 字号”输入 `24` 并失焦。
3. 观察按钮、标题、输入框标签等全局 UI 的文字大小是否变化。
4. 重启应用并再次打开设置页。
...
步骤：
1. 打开设置页，在“文档字体预设”选择 `Georgia`。
2. 回到编辑区，输入一段普通正文后再次观察。
3. 再次打开设置，选择“自定义”，在输入框填入不存在的字体名如 `NonExist Sans`，然后失焦。
4. 回到编辑区再次观察。

预期：
- 选择预设后，文档正文优先使用该字体族
- 切到自定义后，`--yulora-document-font-family` 按输入值更新，文档字体更新为对应值（系统中可用时可见明显差异）

### TC-093 主题家族扫描与刷新

步骤：
1. 关闭应用。
2. 在应用用户数据目录的 `themes` 下新建一个主题家族目录（例如 `themes/demo/light`），并在 `light` 或 `dark` 目录里至少添加 `tokens.css` 与 `ui.css`，写入明显差异样式。
3. 重启应用后打开设置，确认“主题家族”列表出现该目录对应主题。
4. 保持应用运行，删除该目录后返回设置页。
5. 点击“刷新主题”。

预期：
- 启动时会扫描到新建的主题家族目录，并能在“主题家族”里出现
- 运行时不会自动丢失刚创建的主题配置
...
步骤：
1. 打开设置页并确认已加载至少一个主题家族（含 `Yulora 默认` 或社区主题）。
2. 选择不同主题家族，等待界面稳定 1~2 秒。
3. 再切回其他主题家族（例如 `Yulora 默认`）。
4. 若某个主题家族只提供 `dark/` 或 `light/`，切换到其不支持的颜色模式。

预期：
- 切换主题时，当前样式与背景/控件配色与旧主题有区别，并在切回后恢复
- 控制台中 `document.querySelectorAll('link[data-yulora-theme-runtime=\"active\"]')` 的数量始终在 `1~4` 之间，且切换后样式链接指向新主题路径
- 主题缺失的 part 不会报错或持久留存旧样式
- 当前主题家族不支持所选颜色模式时，界面回退到 `Yulora 默认`，并提示“该主题不支持浅色/深色模式”
