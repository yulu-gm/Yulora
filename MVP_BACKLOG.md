...
执行切片：
- [ ] 定义打开文件的 preload bridge 和返回数据结构
- [ ] 在 `src/main/` 中实现文件选择、UTF-8 读取、错误分类
- [ ] 在 `src/renderer/` 中建立“当前文档”状态并把文本显示到界面
- [ ] 为正常读取和失败场景补测试与记录
...
执行切片：
- [ ] 定义保存 / 另存为 bridge 和结果结构
- [ ] 在主进程实现写入、路径更新和失败返回
- [ ] 在 renderer 中建立 dirty 状态、保存动作和状态提示
- [ ] 覆盖保存成功、保存失败、另存为三条路径
...
执行切片：
- [ ] 建立最小 CodeMirror 容器组件和销毁逻辑
- [ ] 把文档状态接到编辑器内容
- [ ] 接入基础快捷键、undo/redo 和变更回调
- [ ] 验证它不破坏打开 / 保存链路
...
状态：未开始
依赖：`TASK-007`、`TASK-010`、`TASK-011`、`TASK-012`、`TASK-013`
...
目标：在测试工作台体系内接入首条 CLI 可触发的打开-编辑-保存-重开冒烟场景。
