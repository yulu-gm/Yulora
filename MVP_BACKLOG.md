...
执行切片：
- [x] 定义链接在阅读态与编辑态的显示策略
- [x] 建立安全的“在系统浏览器打开链接”桥接
- [x] 处理点击、键盘导航和回到编辑态的交互
- [x] 补链接显示和打开行为测试
...
- `recentFiles.maxEntries` 仅保存配置；最近文件列表入口已由 `TASK-006` 接通
...
- [x] 接通 `recentFiles` 列表展示与打开动作（由 TASK-006 完成）
...
主要落点：`src/main/`、`src/preload/`、`src/shared/`、`src/renderer/`。

交付物：
- 主进程持有的 `WorkspaceGraph` 与 `TabSession` 全局工作区真值
- 当前窗口的标签栏 UI 与活动标签切换链路
- `File > New`、`Open...`、`New Window`、外部打开文件、拖入文件的统一标签 / 窗口决策
- 标签页排序、拖出成新窗口与跨窗口移动标签
- 保存、另存为、autosave、外部文件变更、关闭确认迁移到 `tabId` 维度
- 保持单窗口只挂载一个活动编辑器实例的稳定约束

验收：
- 当前窗口可以同时打开多个 Markdown 标签页
- `File > New` 在当前窗口创建新的未保存标签页
- `File > Open...`、拖入文件、外部打开文件在已有文档时默认新增标签页
- 只有 `File > New Window` 才会显式创建新窗口
- 标签页支持排序与拖出成新窗口
