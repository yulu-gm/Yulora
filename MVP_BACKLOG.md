...
目标：把剪贴板图片写入本地资源目录，并插入相对路径 Markdown。
...
目标：建立最小的偏好设置存储，承载 autosave 间隔、最近文件上限、字体、主题等后续 MVP 任务所需的配置项。
...
- 读取/写入/迁移策略
- renderer 消费配置的最小 API
- 默认值与非法值兜底

验收：
- 应用重启后配置保留
- 非法或损坏的配置文件不会导致启动失败
- 后续 TASK-005 / TASK-006 / 主题等能直接基于该存储扩展
